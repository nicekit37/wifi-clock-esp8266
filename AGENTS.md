...
│  ├─ User_Setup.h          # Обёртка → TFT_eSPI/User_Setup.h
...
| `lib/TFT_eSPI/User_Setup.h` | Аппаратно‑зависимые настройки TFT_eSPI (разрешение, пины, `SMOOTH_FONT`). |
...
- При работе с дисплеем учитывайте аппаратные настройки из `lib/TFT_eSPI/User_Setup.h` (корневой `lib/User_Setup.h` — обёртка).
